"""
start_all_bots.py - Run Oneboxx enhanced shipping bot
Handles:
    ...
    while True:
        ...
        for i, proc in enumerate(processes):
            ...
            if not proc.is_alive() and proc.exitcode is not None:
                ...
                log.warning(
                    f"⚠️ {proc.name} died with exit code {proc.exitcode}"
                )

                try:
                    log.info(f"🔄 Restarting {proc.name}...")
                    ...
                    log.error(
                        f"❌ Failed to restart {proc.name}: {e}"
                    )
...
def main():
    """Main function to start enhanced shipping bot."""
    ...
    try:
        ...
        log.info(f"✅ BotEnhanced started (PID: {p1.pid})")
